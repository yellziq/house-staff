...
export const DashboardView = observer((): ReactElement => {
  ...
  useEffect(() => {
    dashboardStore.async.loadDashboard().catch(handleUnknownError);
  ...
  const handleDeleteOrder = async (orderId: string): Promise<void> => {
    try {
      ...
      ordersStore.sync.removeOrder(orderId);
      dashboardStore.sync.decrementActiveOrders();
    ...
    <>
      <section className="dashboard-grid">
        <article className="info-card">
          ...
          <h1>Здравствуйте, {userStore.sync.getUser()?.email}</h1>
          <p>Здесь можно следить за заявками, корзиной и выбранными услугами для дома.</p>
        </article>
        <article className="info-card">
          <h2>{dashboardStore.sync.getSummary().activeOrders}</h2>
          <p>заявок уже отправлено</p>
        </article>
        <article className="info-card">
          <h2>{dashboardStore.sync.getSummary().favoriteCategory}</h2>
          <p>Популярная категория. Сейчас в корзине {cartStore.sync.getItems().length} услуг.</p>
        </article>
        <article className="info-card">
          <h2>{dashboardStore.sync.getSummary().responseTime}</h2>
